...
from evaluate_stage2 import evaluate_stage2
from models.mldbd.mldbd import BdNet, CdNet, Stage2Net
...
from utils.mldbd.losses import loss_bbd
...
def train_model(
    ...
    t4_dataset = SameClassAugmentedDataset('./data/train_data/FCFB/FC', 1, img_scale, (9999, 9999), transforms)
    t3_dataset = SameClassAugmentedDataset('./data/train_data/FCFB/FB', 0, img_scale, (9999, 9999), transforms)
    ...
    for epoch in range(1, epochs + 1):
        ...
        with tqdm(total=n_train, desc=f'Epoch {epoch}/{epochs}', unit='img') as pbar:
            for batch in train_loader:
                images, true_masks = batch['image'], batch['mask'].unsqueeze(1)

                images = images.to(device=device, dtype=torch.float32, memory_format=torch.channels_last)
                true_masks = true_masks.to(device=device, dtype=torch.long)

                with torch.autocast(device.type if device.type != 'mps' else 'cpu', enabled=amp):
                    dbd_pred, boundary_pred, fgs, fc = model(
                        images,
                        t3_dataset[random.randint(0, len(t3_dataset) - 1)]['image'].unsqueeze(0).to(device),
                        t4_dataset[random.randint(0, len(t4_dataset) - 1)]['image'].unsqueeze(0).to(device),
                        true_masks
                    )
                    loss = loss_bbd(dbd_pred, boundary_pred, true_masks.float(), fc, fgs)
                ...
                if division_step > 0:
                    if global_step % division_step == 0:
                        ...
                        val_score = evaluate_stage2(model, val_loader, t3_dataset, t4_dataset, device, amp)

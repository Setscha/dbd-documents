...
from models.mldbd.mldbd import BdNet, CdNet, Stage2Net, MldbdInference
...
def train_model(
    ...
    datasets = [
        AugmentedDataset('./data/cvl/trainset/source/computer', './data/cvl/trainset/gt/computer', img_scale, transforms),
        AugmentedDataset('./data/cvl/trainset/source/handwritten', './data/cvl/trainset/gt/handwritten', img_scale, transforms),
    ]
        ...
        with tqdm(total=n_train, desc=f'Epoch {epoch}/{epochs}', unit='img') as pbar:
            for batch in train_loader:
                ...
                with torch.autocast(device.type if device.type != 'mps' else 'cpu', enabled=amp):
                    ...
                    loss += binary_dice_loss(masks_pred, true_masks.float())
                        ...
                        try:
                            ...
                            experiment.log({
                                ...
                                'masks': {
                                    ...
                                    'pred': wandb.Image(TF.to_pil_image((masks_pred[0] >= 0.5).float().cpu()), 'L'),
                                },
                                'step': global_step,
                                'epoch': epoch,
                                **histograms
...
if __name__ == '__main__':
    ...
    f = './checkpoints/3178879_checkpoint_epoch10_Stage2Net.pth'
    state_dict = torch.load(f, map_location=device)
    model.load_state_dict(state_dict, strict=False)
    logging.info(f'Model loaded from {f}')
